...
export default function BlogPostContent({ title, date, coverSrc, coverAlt = "cover", contentHtml }: BlogPostContentProps) {
  return (
    <section className="py-16 md:py-20">
      <div className="mx-auto max-w-3xl px-4">
        <h1 className="font-display text-4xl md:text-5xl font-semibold">{title}</h1>
        <div className="text-sm text-slate-500 dark:text-slate-400 mt-2">{date}</div>
        <Image src={coverSrc} alt={coverAlt} width={1200} height={700} className="rounded-2xl shadow-soft mt-6" />
